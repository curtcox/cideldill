"""Integration tests for browser sync mode round trips."""
...
def _skip_if_curl_unavailable() -> None:
    if not shutil.which("curl"):
        pytest.skip("curl is required for sync integration tests")
...
class XMLHttpRequest {
  ...
  send(body) {
    const args = ['-s', '-X', this.method, this.url];
    ...
    this.responseText = execFileSync('curl', args, { encoding: 'utf-8' });
  }
}
...
def test_sync_browser_client_pauses_and_resumes(tmp_path: Path) -> None:
    _skip_if_socket_unavailable()
    _skip_if_node_unavailable()
    _skip_if_curl_unavailable()
...
const result = debugCallSync(add, 1, 2);
...
def test_sync_browser_client_modify_args(tmp_path: Path) -> None:
    _skip_if_socket_unavailable()
    _skip_if_node_unavailable()
    _skip_if_curl_unavailable()
...
const result = debugCallSync(sub, 1, 2);
if (result !== 7) throw new Error('modified args not applied');
"""
        proc = _run_node_process(js_path, script)
        ...
        resp = requests.post(
            f"{base_url}/api/paused/{pause_id}/continue",
            json={"action": "modify", "modified_args": [10, 3]},
            timeout=5,
        )
        assert resp.status_code == 200
